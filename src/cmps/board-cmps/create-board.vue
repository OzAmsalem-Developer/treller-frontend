...
<section>
  ...
  <section class="create-board" :style="{background: newBoard.style.background}" ref="createCmp">
      ...
      <input v-model="newBoard.name" placeholder="Board title.." type="text">
        <button class="save-btn" @click="createBoard">Create board</button>
      </main>
        <button @click="isColorOpen = !isColorOpen; isImgOpen = false" class="set-style-btn save-btn">Background color</button>
        <button @click="isImgOpen = !isImgOpen; isColorOpen = false" class="set-style-btn save-btn">Background image</button>

        <board-img-picker  v-if="isImgOpen" @closed="isImgOpen = false" @set-board-img="setBg"/>
        <board-color-picker  v-if="isColorOpen"
         @set-board-color="setBg"
         @closed="isImgOpen = false" />
  </section>
</section>
</template>
...
export default {
    ...
    methods: {
        ...
        async createBoard() {
            ...
            const board = await this.$store.dispatch({type: 'createBoard', board: this.newBoard})
